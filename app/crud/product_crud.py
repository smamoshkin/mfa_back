...
def create_product(db: Session, product: dict):
    ...
    # NOTE: marketplace_sku намеренно НЕ проверяется на уникальность —
    # допустимы несколько товаров с разными sku и одним marketplace_sku
    # (например, одно и то же WB-кольцо под разными внутренними артикулами).
    # Предупреждение о дубле показывается на фронте (ProductModal), создание не блокируется.
...
def update_product(db: Session, product_id: int, product: ProductUpdate):
    ...
    # NOTE: marketplace_sku намеренно НЕ проверяется на уникальность (см. create_product)
