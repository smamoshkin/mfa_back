...
> ⚠️ **СТАТУС 11.09.2026 (TODO №1):** агрегаты переехали с мат.view на обычные
> таблицы `supplier_reports_agg` и `product_margins` (DDL в `db/tables/`),
> пересчёт — код приложения (`app/services/aggregates_service.py`), а не
> REFRESH. Мат.view 01 и 05 (файлы 01_*.sql, 05_*.sql) больше не читаются
> кодом и будут УДАЛЕНЫ после стабилизации (файлы оставлены до сноса).
> Рекламные мат.view (03, 04) остаются действующими — их обновляет
> `wb_advertising_service` (REFRESH рекламных MV → пересчёт таблицы margins).
